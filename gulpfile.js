...
gulp.task('ship-to-gitlab', function (done) {
    ...
    var gitPath = 'https://' + user + ':' + token + `@gitlab.syncfusion.com/content/angularjs-docs`;
    console.log('Clone has been started...!');
    var clone = shelljs.exec('git clone ' + gitPath + ' -b ' + branch + ' ' + `../../../gitlabRepo/angularjs-docs`, {
        silent: false
    });
    if (clone.code !== 0) {
        ...
        for (var changedFileName of changedFileNames.split(',')) {
            ...
            if (changedFileName !== null && changedFileName !== '') {
                ...
                if (fs.existsSync('../angularjs-docs/' + changedFileName)) {
                    // It will update the modified files
                    if (fs.existsSync('../../../gitlabRepo/angularjs-docs/' + changedFileName)) {
                        shelljs.cp('-rf', `../angularjs-docs/` + changedFileName, `../../../gitlabRepo/angularjs-docs/` + changedFileName);
                    }
                    else {
                        // It will update the newly added files
                        if (fs.existsSync('../../../gitlabRepo/angularjs-docs/')) {
                            shelljs.cp('-rf', `../angularjs-docs/` + changedFileName, `../../../gitlabRepo/angularjs-docs/` + changedFileName);
                        }
                    }
                    ...
                    if (fs.existsSync('../../../gitlabRepo/angularjs-docs/' + changedFileName)) {
                        shelljs.rm('-rf', `../../../gitlabRepo/angularjs-docs/` + changedFileName);
        ...
        shelljs.cd(`../../../gitlabRepo/angularjs-docs`);
